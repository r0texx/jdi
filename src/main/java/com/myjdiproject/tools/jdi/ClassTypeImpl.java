...
public final class ClassTypeImpl extends InvokableTypeImpl implements ClassType {
    private record IResult(JDWP.ClassType.InvokeMethod rslt) implements InvocationResult {
        @Override
    ...
    public boolean isEnum() {
        ...
        return superclass != null && "java.lang.Enum".equals(superclass.name());
    }
    ...
    public void setValue(Field field, Value value) throws InvalidTypeException, ClassNotLoadedException {
        ...
        try {
            List<JDWP.ClassType.SetValues.FieldValue> values = new ArrayList<>(1);
            values.add(new JDWP.ClassType.SetValues.FieldValue(field.ref(), ValueImpl.prepareForAssignment(value, (FieldImpl) field)));
            try {
    ...
    PacketStream sendNewInstanceCommand(ThreadReferenceImpl thread, MethodImpl method, List<Value> args, int options) {
        CommandSender sender = () -> JDWP.ClassType.NewInstance.enqueueCommand(vm, ClassTypeImpl.this, thread, method.ref(), args, options);
    ...
    public ObjectReference newInstance(ThreadReference threadIntf, Method methodIntf, List<? extends Value> origArguments, int options) throws InvalidTypeException, ClassNotLoadedException, IncompatibleThreadStateException, InvocationException {
        ...
        try {
            PacketStream stream = sendNewInstanceCommand(thread, method, arguments, options);
            ret = JDWP.ClassType.NewInstance.waitForReply(stream);
        } catch (JDWPException exc) {
    ...
    InvocationResult waitForReply(PacketStream stream) throws JDWPException {
        return new IResult(JDWP.ClassType.InvokeMethod.waitForReply(stream));
    }
